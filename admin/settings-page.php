...
function fontx_settings_page_callback() {
    ...
    sort($font_names, SORT_NATURAL | SORT_FLAG_CASE);

    $current = get_option('fontx_selected_font', '__default__');

    if (!empty($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST' && check_admin_referer('fontx_save_font', 'fontx_nonce')) {
        if (isset($_POST['fontx_font'])) {
            $font = sanitize_text_field(wp_unslash($_POST['fontx_font']));
